...
// Coletar dados da solicitação AJAX PRODUTO
 if (isset($_POST['produtoId'])) {
    ...
    if(isset($_POST['url_img1'])){
        $foto = filter_var($_POST['url_img1'], FILTER_SANITIZE_URL);
    } else {
        $foto = null;
    }
    ...
    $categoria = $p->pegaIdCategoria($categoriaNome);
     ...
     // Realizar a atualização no banco de dados
     try {
         $result = $p->atualizarProdutoModal($produtoId, $nome, $desc, $preco, $desconto, $categoria, $estoque, $foto, $status);
         echo json_encode($result);
     } catch (Exception $e) {
         echo json_encode(['error' => $e->getMessage()]);
     }
